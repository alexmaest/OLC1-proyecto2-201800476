...
| EXP {$$ = [$1];}
;
FUNCION : Tx_Id '(' PARAMETROS ')' ':' TIPO_VAR BLOQUE {$$=new Funcion($1,$3,$6,$7,1);}
| Tx_Id '(' ')' ':' TIPO_VAR BLOQUE {$$=new Funcion($1,[],$5,$6,2);}
| Tx_Id '(' PARAMETROS ')' BLOQUE {$$=new Funcion($1,$3,null,$5,3);}
| Tx_Id '(' ')' BLOQUE {$$=new Funcion($1,[],null,$4,4);}
;
...
| DOWHILE {$$=$1;}
;
IF : 'if' '(' EXP ')' BLOQUE ELSE {$$ = new If($3,$5,$6);}
;
ELSE :'else' BLOQUE{$$ = $2;}
| 'else' IF {$$ = $2;}
| /*epsilon*/ {$$ = null;}
;
SWITCH : 'switch' '(' EXP ')' BLOQUE_SWITCH {$$ = new Switch($3,$5);}
;
CASE_LIST : CASE_LIST 'case' EXP ':' INSTRUCCIONES_L {
    var bloque = new Bloque($5);
    var singleCase =  new Case($3, bloque);
    $1.push(singleCase);
    $$ = $1;
}
| 'case' EXP ':' INSTRUCCIONES_L {
    var bloque = new Bloque($4);
    var singleCase =  new Case($2, bloque);
    $$ = [singleCase];
}
;
DEFAULT : 'default' ':' INSTRUCCIONES_L {
    var bloque  = new Bloque($3);
    var singleCase =  new Case(null, bloque);
    $$ = singleCase;
}
;
WHILE : 'while' '(' EXP ')' BLOQUE {$$ = new While($3,$5);}
;
FOR : 'for' '(' VARIABLE EXP ';' ASIGNACION ')' BLOQUE {$$ = new For($3,$4,$6,$8);}
| 'for' '(' ASIGNACION ';' EXP ';' ASIGNACION ')' BLOQUE {$$ = new For($3,$5,$7,$9);}
;
DOWHILE : 'do' BLOQUE 'while' '(' EXP ')' ';' {$$ = new DoWhile($2,$5);}
;
...
| Tx_Id '--' {$$=new OperacionSimplificada($1,'--');}
;
ID_LIST : ID_LIST ',' Tx_Id {$1.push($3);$$ = $1;}
| Tx_Id {$$=[$1];}
;
TIPO_VAR : 'int' {$$=$1;}
| 'double' {$$=$1;}
